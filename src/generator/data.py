...
def generate_client_data(num_clients) -> list:
    clients = []
...
def generate_address_data(num_addresses) -> list:
    addresses = []
...
def generate_account_data(num_accounts) -> list:
    accounts = []
...
def generate_card_data(num_cards) -> list:
    cards = []
...
def generate_transaction_data(num_transactions) -> list:
    transactions = []
...
def generate_credit_contract_data(num_contracts) -> list:
    contracts = []
...
def rename_csvs(base_tmp_path, final_path):
    ...
    for file in os.listdir(base_tmp_path):
        ...
        csv_files = glob.glob(f"{csv_path}/*.csv")[0]
        if not csv_files:
            raise FileNotFoundError(f"Nenhum CSV encontrado em {csv_path}")
